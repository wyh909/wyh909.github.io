...
(function(){
	...
	function getUser(){
		...
		// 	callback: function(json){
				if(json.ret ==1){
					...
					'<div class="logined">',
						'<div class="welcome">',
							'<span class="username">' + (data.name||'zhangsan@163.com') + '</span>，欢迎使用',
						'</div>',
						'<div class="user-opt">',
	...
	navHtml.push([
		...
		'<span class="line">|</span>',
		'<a href="'+rootUrl+'gerenyuyue.html" data-id="10">个人预约</a>',
		'</div>'
	].join(''));
	var selectBox = [];
	if(href.indexOf('kehuguanli')>-1 || href.indexOf('index')>-1){
		selectBox.push([
	...
	searchHtml.push([
		'<span class="select icons" data-id="'+((href.indexOf('kehuguanli')>-1||href.indexOf('index')>-1)?'carOwner':'name')+'">客户名称</span>',
	...
	$('div.header').on('click', 'a.modifypwd', function(e){
		e.stopPropagation();
		...
		if($dialog.size()>0){
			...
			$('body').append('<div class="dialog dialog-modify"></div>');
			$dialog = $('div.dialog-modify');
			$dialog.dialog({
				title: '修改密码',
	...
	function blurEvent(input){ 
        ...
        // 如果是登录，需要截取掉头一位
        if($curInput.parent().hasClass('login-item')){
	...
	getUser();
	checkVal();
	console.log('新加的项目：注册、找回密码、推荐商品管理、雇员管理、车辆品牌管理、车辆类型管理、部门管理、供应商管理、服务商等模块');
	console.log('已结5k，还有7.5k尾款未结清，不包含新添加的项目（15个页面），每页按500计算');
})();